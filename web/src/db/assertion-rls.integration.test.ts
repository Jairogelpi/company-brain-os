...
describe.skipIf(!databaseUrl)("assertion ledger RLS", () => {
	beforeAll(async () => {
		...
		await admin.query("insert into assertions (id, organization_id, subject_entity_id, predicate, status, proposed_by, confidence_class, metadata) values ($1, $2, 'person', 'MASTERS', 'approved', 'owner', 'verified', '{}'::jsonb)", [`assertion-${suffix}`, orgA]);
	...
	it("denies cross-tenant reads and writes", async () => {
		...
		await expect(app.query("insert into assertions (id, organization_id, subject_entity_id, predicate, status, proposed_by, confidence_class, metadata) values ($1, $2, 'person', 'MASTERS', 'approved', 'owner', 'verified', '{}'::jsonb)", [`cross-${suffix}`, orgA])).rejects.toThrow(/row-level security/i);
